...
Use `jv_lifecycle_transition` — it moves the card AND updates labels atomically in one call:

```json
{ "name": "jv_lifecycle_transition", "arguments": { "cardId": "...", "transition": "pickup" } }
```

Available transitions: `pickup` · `complete` · `block` · `unblock`

Only fall back to manual `jv_move_card` + `jv_update_card_details` if `jv_lifecycle_transition` fails.
...
|---|---|
| `jv_add_comment` | `{ id }` |
| `jv_lifecycle_transition` | `{ id, transition, idList, labelsAdded, labelsRemoved }` |
